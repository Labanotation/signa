...
app.use(
    session({
        ...
        store: new PouchSession(
            `http://${process.env.DB_USER}:${process.env.DB_PASS}@${process.env.DB_HOST}:${process.env.DB_PORT}/${process.env.DB_SESSIONS_NAME}`
        )
    })
);
        ...
        if (errors.length > 0) {
            ...
            if (savedUser && savedUser[0] && savedUser[0].ok) {
                const accessToken = jwt.sign(
                    { login: pendingUser.login, email: pendingUser.email, id: pendingUser.id, action: 'verify' },
...
app.get('/signupvalidation/:token', async (req, res) => {
    jwt.verify(req.params.token, process.env.JWT_SECRET, async (err, userData) => {
        if (!err && userData.login && userData.email && userData.id && userData.action && userData.action === 'verify') {
            ...
            if (
                pendingUser &&
                pendingUser.email === userData.email &&
                pendingUser.id === userData.id &&
                pendingUser.verified === false &&
                pendingUser.signuptoken === req.params.token
            ) {
    ...
    jwt.verify(req.params.token, process.env.JWT_SECRET, async (err, userData) => {
        if (!err && userData.login && userData.email && userData.id && userData.action && userData.action === 'reset') {
            ...
            if (errors.length > 0) {
                ...
                if (
                    !existingUserByLogin ||
                    existingUserByLogin.id !== userData.id ||
                    existingUserByLogin.email !== userData.email ||
                    existingUserByLogin.signuptoken !== req.params.token
                ) {
                    res.render('forgotpassword', {
                        title: 'Signa > ' + i18n.__('forgotpassword.title'),
                        lang: req.getLocale(),
...
app.get('/dashboard', (req, res) => {
    if (req.isAuthenticated()) {
        ...
        res.render('dashboard', {
            ...
            occupations: User.getOccupations(i18n, req.getLocale())
        });
    } else {
        res.redirect('/login');
    }
